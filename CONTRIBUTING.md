...
| Your edit to one of the extension's `.js` files doesn't seem to apply | Rebuild (⌘R), then reload the page. If it's `background.js`, quit and reopen Safari (and re-tick Allow Unsigned Extensions). |
...
| Path | What |
|---|---|
| `app/…/MediaKeyGuardForSafari Extension/Resources/` | **The extension itself** — manifest, scripts, icons. Almost every change you'll want to make is here. Called *the extension's `Resources/` folder* below. |
...
| `tools/` | Icon generator, version stamper, screenshot renderer, install cleaner. |

Inside the extension's `Resources/` folder:
...
The files Xcode shows you under **Resources** are the real, tracked ones.
Edit them there or in any editor — same files either way. **Edit, press ⌘R,
done.**

One thing to remember: adding a **new** file to that folder also needs it
registered in the Xcode project (drag it in, targeting the extension target,
or follow the existing entries in `project.pbxproj`). Xcode won't bundle a
file it has no reference for. New icons inside `images/` need nothing —
that's a folder reference.

## The edit loop

1. Edit a file in the extension's `Resources/` folder.
2. ⌘R in Xcode.
...
```bash
EXT="app/MediaKeyGuardForSafari/MediaKeyGuardForSafari Extension/Resources"
for f in "$EXT"/*.js; do node --check "$f"; done
...
```bash
EXT="app/MediaKeyGuardForSafari/MediaKeyGuardForSafari Extension/Resources"
git add "$EXT/page.js" "$EXT/background.js"
...
```bash
git add "app/MediaKeyGuardForSafari/MediaKeyGuardForSafari Extension/Resources/page.js"
...
- Never commit signing changes or bundle identifiers.
...
The stamp build phase needs `ENABLE_USER_SCRIPT_SANDBOXING = NO` on the
extension target (set there, not project-wide) — the script writes to a source
file, outside the build directory.

Run it by hand only if you want `manifest.json` re-stamped without a build:

```bash
tools/stamp-version.sh
```
...
app/MediaKeyGuardForSafari/Version.xcconfig
  MARKETING_VERSION        -> app + appex CFBundleShortVersionString
                           -> manifest.json "version" (stamped by
                              stamp-version.sh, which is why the extension
...
the stamp phase writes it into `manifest.json`.
...
`manifest.json` only accepts dot-separated integers, so the stamp script
